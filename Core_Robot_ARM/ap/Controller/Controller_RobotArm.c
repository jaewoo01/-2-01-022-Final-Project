			...
			if (dataEvt.status == osEventMessage){
				...
				if(pNewState != NULL){
					...
					if (stat != osOK) {
						...
						//printf("Robot Queue Full! Leak prevented.\n"); //디버깅용
					}
				}
...
void Controller_CalcIK(RobotArm_t *pRobot)
{
	...
	float rel_x_cm = (raw_x / FPGA_MAX_X) * REAL_FIELD_WIDTH_X;
	float rel_y_cm = (1.0f - (raw_y / FPGA_MAX_Y)) * REAL_FIELD_HEIGHT_Y;
	...
	printf("B = %02x S= %02x E = %02x\r\n", pRobot->angleBase, pRobot->angleShoulder, pRobot->angleElbow);
}
